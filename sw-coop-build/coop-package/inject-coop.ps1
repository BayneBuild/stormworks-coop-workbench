<#
  Stormworks co-op mod - connect & inject (peer side).
  1) shows YOUR SteamID64 (give it to your partner)
  2) asks for your PARTNER's SteamID64 -> writes coop-peer.txt
  3) injects coop.dll (in THIS folder) into a running stormworks64.exe
  Just run inject-coop.bat.
#>
$ErrorActionPreference = 'Stop'
$here = Split-Path -Parent $MyInvocation.MyCommand.Path
$Dll  = Join-Path $here 'coop.dll'
$PeerFile = Join-Path $here 'coop-peer.txt'
if (-not (Test-Path $Dll)) { Write-Host "coop.dll not found next to this script." -ForegroundColor Red; Read-Host 'Press Enter'; exit 1 }
...
Write-Host "==== Stormworks Co-op ====" -ForegroundColor Cyan
$my = Get-MySteamID64
if ($my) {
...
while ($true) {
  ...
  if ($inp -eq '') { Write-Host "No partner set - watch-only (you'll receive their blocks; yours won't send)." -ForegroundColor Yellow; break }
...
public static class Inj {
  ...
  public static string Run(int pid, string dllPath) {
    ...
    try {
      ...
      CloseHandle(th);
      return code!=0 ? "OK coop.dll injected." : "WARN thread ran but LoadLibrary returned 0";
    } finally { CloseHandle(h); }
  }
}
"@ -Language CSharp

Write-Host "Injecting into Stormworks (PID $($p.Id)) ..." -ForegroundColor Cyan
$r = [Inj]::Run($p.Id, $Dll)
if ($r -like 'OK*') { Write-Host $r -ForegroundColor Green } else { Write-Host $r -ForegroundColor Yellow }
Start-Sleep -Milliseconds 500
$log = Join-Path $here 'coop-log.txt'
if (Test-Path $log) { Write-Host "`nStatus:" -ForegroundColor Cyan; Get-Content $log -Tail 5 }
Write-Host "`nIn the workbench: single-click ONE block to arm, then build - your blocks appear on your partner's screen and theirs on yours." -ForegroundColor DarkGray
